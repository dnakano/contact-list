...
function ContactDialog({
  ...
  function validateEmail(id, email) {
    const emailRegExp = /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,4})+$/u;

    // Validate format
    if (email && !emailRegExp.test(email)) {
      return 'Invalid email format. E.g. you@email.com';
    }
  ...
  function handleChange({ target }) {
    ...
    if (name === 'photo' && isValidPhoto(files)) {
      ...
      // Hide label text
      const label = document.querySelector('.ContactForm-photo label');
  ...
  const config = {
    className: 'ContactDialog-modal',
    position: 'top',
    showClose: true,
    size: 'full',
    title: msg,
    onCloseClick: handleResetClick,
  };
    ...
    <Modal config={config}>
      {
        isDelete ? (
          <ContactFormButtons
            ...
            buttonClassName="ContactFormButtons-warn"
